...
export function hasCriticalEscalationTrigger(
  ...
  return (
    ...
    alertCase.helpEvent.active ||
    (alertCase.patientStatus.spo2 < 92 && alertCase.patientStatus.activityDropPercent > 60)
  );
}
...
function packetEmergencyStatus(packet: AlertCase["lowDataPacket"], isCritical: boolean): EmergencyFlowStatus {
  if (!packet) return isCritical ? "active" : "pending";
  ...
  return isCritical ? "active" : "pending";
}

function consoleAckStatus(packet: AlertCase["lowDataPacket"], isCritical: boolean): EmergencyFlowStatus {
  if (!isCritical) return "pending";
  ...
  return [
    {
      ...
      meta: [
        { label: "condition", value: "riskScore >= 90 / helpEvent / SpO2 + activity" },
    ...
    },
    {
      ...
      status: packetEmergencyStatus(packet, isCritical),
      meta: [
    ...
    },
    {
      ...
      status: consoleAckStatus(packet, isCritical),
      meta: [
...
export function generateCareEvent(alertCase: AlertCase, kind?: GeneratedCareEventKind): AlertCase {
  ...
  } else if (["bedside_pressed", "spo2_drop", "activity_drop", "packet_delay"].includes(selectedKind)) {
    escalationFlowVisible = true;
  }
